...
p2_given_p1_only_once <- function(absolute_p2, absolute_p1, s, h, N, generations_in_between) {
...
p2_given_depth_p1_reads_p1_matrix <- function(absolute_p2, depth_p1, reads_p1, s, h, N, generations_in_between, sample_method) {
    ...
    if (length(absolute_p2) == 1) {
        ...
        prob_p2_given_p1_matrix <- p2_given_p1(0:(2*N), 0:(2*N), s = s, h = h, N = N, generations_in_between = generations_in_between)
...
## singular version
reads_p2_given_reads_p1_singular <- function(reads_p2, depth_p2, reads_p1, depth_p1, s, h, N, generations_in_between, sample_method) {
    if (length(reads_p2) != 1) {
    ...
    if (((reads_p2 == depth_p2) & (reads_p1 == depth_p1)) | ((reads_p2 == 0) & (reads_p1 == 0))) {
        ...
        temp_prob <- sum(reads_p2_given_p2(reads_p2, depth_p2, absolute_p2, N, sample_method = sample_method) * p2_given_depth_p1_reads_p1_matrix(absolute_p2, depth_p1, reads_p1, s, h, N, generations_in_between, sample_method = sample_method))
...
reads_p2_given_reads_p1 <- function(data, s, h, N, sample_method) {
    if (length(data) != 5) {
...
##    print(N)
    ...
    if(max(length(reads_p2), length(depth_p2), length(reads_p1), length(depth_p1)) == 1) {
        ...
        prob <- reads_p2_given_reads_p1_singular(reads_p2, depth_p2, reads_p1, depth_p1, s, h, N, generations_in_between, sample_method = sample_method)
...
log_likelihood_per_s_h <- function(parameters, seq_data, N, sample_method) {
    if (is.matrix(seq_data) == TRUE) {
        ...
        ##        sum(apply(X = seq_data, MARGIN = 1, FUN = function(data) reads_p2_given_reads_p1(data, s = s, h = h, N = 100)))
                sum(apply(X = seq_data, MARGIN = 1, FUN = function(data) reads_p2_given_reads_p1(data, s = s, h = h, N = N, sample_method = sample_method)))
